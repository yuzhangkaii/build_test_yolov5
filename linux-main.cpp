...
YOLO::YOLO(Net_config config)
{
    ...
    // 将模型数据写入临时文件
    std::ofstream model_file("temp_model.onnx", std::ios::binary);
    model_file.write(reinterpret_cast<const char*>(ee_onnx), ee_onnx_len);
    model_file.close();

    string model_path = "temp_model.onnx";
    ...
    this->num_class = this->class_names.size();
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        cout << "Usage: ./yolov5 输入文件 算法阈值 视频帧率 输出文件夹" << endl;
        cout << "Example: ./yolov5 1.mp4 6 30 test" << endl;
        return 1;
    }
